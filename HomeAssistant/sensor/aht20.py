...
class ATH20:
    ...
    def __init__(self, i2c) -> None:
        ...
        self._i2c.writeto_mem(self.address, self.Address.INIT, b'\x08\x00')
        time.sleep(0.5)
        retry_count = 0
        while not self.read_cal_enable():
            self._i2c.writeto_mem(self.address, self.Address.SoftReset, b'')
            time.sleep(0.2)
            self._i2c.writeto_mem(self.address, self.Address.INIT, b'\x08\x00')
            retry_count += 1
            if retry_count >= 10:
                print('ATH20 Not detected')
            time.sleep(0.5)

    def read_status(self) -> int:
        return self._i2c.readfrom_mem(self.address, 0x00, 1)[0]
    ...
    def read_ct_data(self):
        self._i2c.writeto_mem(self.address, self.Address.StartTest, b'\x33\x00')
        time.sleep(0.075)
        retry_count = 0
